function redirectToCourse(courseId) {
    window.location.href = `http://localhost:5000/course/${courseId}`;
}

function renderWishedCourse(course, ratingPoint) {
    $('#wishlist').find('.row').append(
        `
        <div class='col-lg-3 col-sm-6 mb-5'>
            <div class='card mb-2 border-primary rounded-0 hover-shadow position-relative' id='${course.idCourse}'>
                <img class='card-img-top' src='/images/hero-bg.jpg' alt='Course image thumb'>
                <div class='card-body'>
                    <h4 class='course-title'>${course.title}</h4>
                    <ul class="list-inline my-2">
                    ${course.instructors.map(instructor => (
                        `<li class="list-inline-item">${instructor.name}</li>`
                    ))}
                    </ul>
                    <div class="course-rating mb-2">
                        <p class="rating-point">${ratingPoint}</p>
            ...
            <div class='loader-wrapper position-absolute'>
                <i class="fas fa-heart active" data-toggle="tooltip" data-placement="top" title="Remove from Wishlist"></i>
            </div>
        </div>
        `
    );
...
$(document).ready(function() {
    ...
    if (wishlist) {
        ...
        if (wishlist.length === 0) {
            $('#wishlist').find('.row').append("<a href='/' type='button' class='btn btn-primary mx-auto'>Browse courses now</button>")
        }
        ...
        wishlist.map(course => {
            ...
            renderWishedCourse(course, ratingPoint.toFixed(2));

            $(`#${course.idCourse}`).click(function() {
                redirectToCourse(course.idCourse);
            });
            ...
            $('.loader-wrapper').click(function() {
                ...
                $('.loader-wrapper').toggleClass('loading');
                $('.loader-wrapper').find('.fa-heart').addClass('loading');
                ...
                if ($(this).find('.fa-heart').hasClass('active')) {
                    $.ajax({
                        ...
                        if (msg === 'success') {
                            $('.loader-wrapper').toggleClass('loading');
                            $('.loader-wrapper').find('.fa-heart').removeClass('loading active');
                        }
                    });
                } else {
                    $.ajax({
                        ...
                        if (msg === 'success') {
                            $('.loader-wrapper').toggleClass('loading');
                            $('.loader-wrapper').find('.fa-heart').removeClass('loading');
                            $('.loader-wrapper').find('.fa-heart').addClass('active');
                        }
                    });
                }
            });
